 ...
 * Modifications Copyright (c) 2020, Texas Instruments Incorporated.
 * All rights reserved.
...
#ifdef CLI_MAX_LINE_LENGTH
#define MBED_CONF_CMDLINE_MAX_LINE_LENGTH CLI_MAX_LINE_LENGTH
#endif
...
#if IS_ROOT
#define MBED_CONF_CMDLINE_BOOT_MESSAGE "Simple UDP Meter Root\r\n"
#else
#define MBED_CONF_CMDLINE_BOOT_MESSAGE "Simple UDP Meter Node\r\n"
#endif
...
#define LEFT ESCAPE("[D")
#define N_LEFT ESCAPE("[%dD")
...
static int cmd_run(char* string_ptr)
{
    ...
    while (isspace((unsigned char)*string_ptr) && *string_ptr != '\n' && *string_ptr != 0) {
    ...
    if (argc == 2 && (cmd_has_option(argc, argv, "h") || cmd_parameter_index(argc, argv, "--help") > 0)) {
        MEM_FREE(command_str);
...
static void cmd_arrow_right()
{
    ...
    */
    if ((cmd.escape_index == 1 && cmd.escape[0] == 'O') || (cmd.escape_index == 4 && strncmp(cmd.escape + 1, "1;5", 3) == 0)) {
...
void cmd_char_input(int16_t u_data)
{
    ...
    if (u_data == '\r' || u_data == '\n') {
        ...
        if ((strlen(cmd.input) >= MBED_CONF_CMDLINE_MAX_LINE_LENGTH - 1) || (cmd.cursor >= MBED_CONF_CMDLINE_MAX_LINE_LENGTH - 1)) {
...
{
    if ((*cmdline_get_prompt() != ' ' && *cmdline_get_prompt() != 0)) {
        cmd_printf("%s", cmdline_get_prompt());
    }
}
...
void cmd_output(void)
{
    ...
    if (cmd.vt100_on && cmd.idle) {
        ...
        if (onboot == 0) {
            cmd_printf(CLEAR_ENTIRE_LINE); /* Clear line */
            /* go back the entire length of the word after clearing the screen */
            print_boot_message();
            onboot = 1;
        } else {
            if (cmd.input[0] == 0) {
            ...
            /* Move the cursor back as far as it was last time */
            move_uart_cursor_left(last_length);
            cmd_printf("%s ", cmd.input);
            move_uart_cursor_left(curpos);

            /* save the current length of the input so we
             * know how far left to go next time.
             */
            last_length = (int)strlen(cmd.input);
        }
    }
#endif
}
void move_uart_cursor_left(int howfar)
{
    if (howfar == 0) {
        // return and don't move cursor
        return;
    }
    cmd_printf(N_LEFT, howfar);
}
...
int replace_alias(char* str, const char* old_str, const char* new_str)
{
    ...
    if ((strncmp(str, old_str, old_len) == 0) && ((str[old_len] == ' ') || (str[old_len] == 0) || (str[old_len] == ';') || (str[old_len] == '&'))) {
...
void cmd_variable_add_int(char* variable, int value)
{
    ...
    if (variable_ptr->value.ptr != NULL && variable_ptr->type == VALUE_TYPE_STR) {
        // free memory
        MEM_FREE(variable_ptr->value.ptr);
    }
...
{
    if ((strncmp(command, "alias", 5) == 0) || (strcmp(command, "echo") == 0) || (strcmp(command, "set") == 0) || (strcmp(command, "clear") == 0) || (strcmp(command, "help") == 0)) {
        return true;
    }
    return false;
}
...
bool cmd_parameter_bool(int argc, char* argv[], const char* key, bool* value)
{
    ...
    if (i > 0) {
        if (argc > (i + 1)) {
            if (strcmp(argv[i + 1], "on") == 0 || strcmp(argv[i + 1], "1") == 0 || strcmp(argv[i + 1], "true") == 0 || strcmp(argv[i + 1], "enable") == 0 || strcmp(argv[i + 1], "allow") == 0) {
                *value = true;
            } else {
                *value = false;
            }
    ...
    if (i > 0) {
        if (argc > (i + 1)) {
            if (strchr(argv[i + 1], ',') != 0) {
                ...
                uint8_t buf[8];
                if (strlen(argv[i + 1]) == 23 && string_to_bytes(argv[i + 1], buf, 8) == 0) {
