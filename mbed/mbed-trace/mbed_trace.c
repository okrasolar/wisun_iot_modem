...
int mbed_trace_init(void)
{
    ...
    if (m_trace.line == NULL || m_trace.tmp_data == NULL || m_trace.filters_exclude == NULL || m_trace.filters_include == NULL) {
...
static int8_t mbed_trace_skip(int8_t dlevel, const char* grp)
{
    if (dlevel >= 0 && grp != 0) {
        ...
        if (m_trace.filters_exclude[0] != '\0' && strstr(m_trace.filters_exclude, grp) != 0) {
            // grp was in exclude list
            return 1;
        }
        if (m_trace.filters_include[0] != '\0' && strstr(m_trace.filters_include, grp) == 0) {
