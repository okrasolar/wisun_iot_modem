...
#ifdef DBG_WISUN
    if(!block_ptr){
        wisunDbg.mem_alloc_failures++;
    }
    else
    {
        wisunDbg.mem_bytes_allocated += (data_size + 2) * sizeof(ns_mem_word_size_t);
        if(wisunDbg.mem_bytes_allocated > wisunDbg. max_mem_bytes_allocated)
        {
            wisunDbg. max_mem_bytes_allocated = wisunDbg.mem_bytes_allocated;
        }
    }
#endif
...
/* Memory Leak Debug */
#ifndef DBG_WISUN_MEM_ALLOC_TRACE
void *ns_dyn_mem_alloc(ns_mem_block_size_t alloc_size)
{
    return ns_mem_alloc(default_book, alloc_size);
}
#else
void *ns_dyn_mem_alloc_new(char* str, ns_mem_block_size_t alloc_size)
{
    tr_debug("ALLOC:\t%s\tsize\t%d", str, alloc_size);
    return ns_mem_alloc(default_book, alloc_size);
}
#endif
...
#ifdef DBG_WISUN
            wisunDbg.mem_bytes_allocated -= (size + 2) * sizeof(ns_mem_word_size_t);
            //tr_debug("FREE:\tsize\t%d", (size + 2) * sizeof(ns_mem_word_size_t));
#endif
