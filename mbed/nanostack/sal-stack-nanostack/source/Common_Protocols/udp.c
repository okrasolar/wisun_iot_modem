...
#ifdef WISUN_NCP_ENABLE
static bool wisun_ncp_filter_udp_port(uint16_t port_num)
{
    uint8_t port_idx;
    bool filter = false;
    if (!ncp_enabled)
    ...
    for (port_idx = 0; port_idx < MAX_FILTER_PORTS; port_idx++)
    {
        if(wisun_ncp_filter_ports[port_idx] == port_num)
        {
            filter = true;
        }
    }
