...
drop:
     ...
     */
    int16_t exthdr_result = 0; //TI fix
    buf = ipv6_get_exthdrs(buf, IPV6_EXTHDR_INSERT, &exthdr_result);
    if (!buf) {
        tr_info("IP:ipv6_get_exthdrs returns NULL");
        return NULL;
    }
    if (exthdr_result < 0) {
        tr_error("ipv6_down: exthdr_result (%d) < 0", exthdr_result);
        goto drop;
    }
    ...
    if ((buf = buffer_headroom(buf, ip_hdr_len)) == NULL) {
        tr_error("ipv6_down: NULL buffer_headroom");
        return (buf);
    }
...
otError nanostack_process_stream_net_from_host(uint8_t* framePtr, uint16_t payload_length)
{
    ...
    buf->interface = global_interface_ptr; //protocol_stack_interface_info_get_by_id(IF_6LoWPAN);
    if(!buf->interface)
    {
        buffer_free(buf);
        /** Wi-SUN protocol interface is not up yet **/
