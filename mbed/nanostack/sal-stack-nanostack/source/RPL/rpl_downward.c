...
//TI Added change to allow for address registration retries
#define MAX_ADDR_REG_FAIL_LIMIT 3
...
uint16_t dbg_con_dev_list_size = 0;
uint16_t dbg_num_dao_created = 0;
uint16_t dbg_num_dao_deleted = 0;
...
{
    /* For each notified parent, send a No-Path DAO */
...
void rpl_instance_dao_trigger(rpl_instance_t *instance, uint16_t delay)
{
    ...
    if (instance->delay_dao_timer == 0 || instance->delay_dao_timer > delay) {
        ...
        tr_extra_debug("DAO trigger %d", delay);
    }
}
...
void rpl_instance_send_dao_update(rpl_instance_t *instance)
{
    ...
    if (instance->pending_neighbour_confirmation) {
        ...
        tr_extra_debug("\n DAO delayed due to pending neighbor confirmation");
        return;
    }
    ...
    if (instance->dao_in_transit) {
        ...
        tr_extra_debug("\n DAO delayed due to dao_in_transit");
        return;
    }
...
static rpl_dao_root_transit_t *rpl_downward_add_root_transit(rpl_dao_target_t *target, const uint8_t parent[16], uint8_t path_control)
{
    ...
    {
        tr_info("Device %s uses %s as parent", trace_ipv6(target->prefix), parent);
    }
...
kill_candidate:
    ...
    {
        tr_debug("rpl_downward_topo_sort_break_loop: NULL pointer");
    }
}
...
#ifndef UNIT_TEST_MAX_NW_CAPACITY
...
bool rpl_instance_address_registration_done(protocol_interface_info_entry_t *interface, rpl_instance_t *instance, rpl_neighbour_t *neighbour, uint8_t status)
{
    ...
    if (status != SOCKET_TX_DONE) {
        if (neighbour->addr_reg_failures > MAX_ADDR_REG_FAIL_LIMIT) {
        ...
        tr_warn("Address registration ACK fail retries count = %d ", neighbour->addr_reg_failures);
