...
rpl_instance_t* global_rpl_instance;
#endif
...
static void rpl_control_etx_change_callback(int8_t  nwk_id, uint16_t previous_etx, uint16_t current_etx, uint8_t attribute_index, const uint8_t *mac64)
{
    ...
    tr_debug("Triggering parent selection due to ETX %s on neigh index %u, etx %u, prev etx %u", better ? "better" : "worse", attribute_index, current_etx, previous_etx);
    ...
    ns_list_foreach(rpl_instance_t, instance, &domain->instances) {
        ...
        if (rpl_instance_am_root(instance)) {
            ...
            if (better) {
#else
            if (better && rpl_better_parent_count < MAX_BETTER_PARENT_CHECK) {
                rpl_better_parent_count++;
#endif
...
#ifndef FEATURE_WISUN_SUPPORT
    tr_extra_debug("\n call rpl_instance_neighbours_changed");
...
void rpl_control_transmit(rpl_domain_t *domain, protocol_interface_info_entry_t *cur, uint8_t code, buffer_t *buf, const uint8_t *dst)
{
    ...
    tr_debug("\n send DAO to dst %s", trace_ipv6(buf->dst_sa.address));
...
#ifdef FEATURE_WISUN_SUPPORT
        prefix->lifetime = 0xFFFFFFFF;
        prefix->preftime = 0xFFFFFFFF;
#endif
...
static buffer_t *rpl_control_dao_ack_handler(protocol_interface_info_entry_t *cur, rpl_domain_t *domain, buffer_t *buf)
{
    ...
    /* Used to send unicast packets later on */
    sent_dao = true;
...
const uint8_t *rpl_control_preferred_parent_addr(const rpl_instance_t *instance, bool global)
{
    ...
    tr_extra_debug("rpl_control_preferred_parent_addr called with global %d", global);
    if (!parent) {
        tr_extra_debug("\n rpl preferred parent not found");
        return NULL;
    }
    if (global) {
        tr_extra_debug("\n rpl_control_preferred_parent_addr global : %s", trace_ipv6(parent));
        return rpl_neighbour_global_address(parent);
    } else {
        tr_extra_debug("\n rpl_control_preferred_parent_addr local : %s", trace_ipv6(parent));
