...
protocol_interface_info_entry_t *global_interface_ptr;
#endif
...
static void protocol_buffer_poll(buffer_t *b)
{
    ...
    if (cur && cur->if_stack_buffer_handler) {
        tr_extra_debug("protocol_buffer_poll: send to  if_stack_buffer_handler");
        cur->if_stack_buffer_handler(b);
        return;
    }

error:
    tr_extra_debug("protocol_buffer_poll: socket fail ");
