...
uint_fast8_t mac_helper_frame_overhead(protocol_interface_info_entry_t *cur, const buffer_t *buf)
{
    uint_fast8_t length;
#ifdef FEATURE_TIMAC_SUPPORT
    /* 8 bytes src address, 8 dst address, 2 frame control, 1 sequence, 2 pan-id, 4 FCS */
    length = 25;

    if (buf->src_sa.addr_type == ADDR_NONE) {
        if (cur->mac_parameters->shortAdressValid) {
            length -= 8; //Cut 6 bytes from src address
        }
