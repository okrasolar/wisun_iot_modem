...
#ifdef FEATURE_TIMAC_SUPPORT
static uint8_t last_sent_async_frame_type = 0xFF;
#endif
...
static void ws_llc_mac_confirm_cb(const mac_api_t *api, const mcps_data_conf_t *data, const mcps_data_conf_payload_t *conf_data)
{
    ...
    if(!conf_data)
    {
        base->asynch_confirm(base->interface_ptr, last_sent_async_frame_type);
        last_sent_async_frame_type = 0xFF;
        /* reset async frame type */
        return;
    }
#endif
...
static void ws_llc_mac_indication_cb(const mac_api_t *api, const mcps_data_ind_t *data, const mcps_data_ie_list_t *ie_ext)
{
...
#ifdef DBG_WISUN
    wisunDbg.ind[ws_utt.message_type]++;
#endif
    ...
    if (ws_utt.message_type == WS_FT_DATA) {
        tr_extra_debug("calling ws_llc_data_indication_cb");
        ws_llc_data_indication_cb(api, data, ie_ext, ws_utt);
        return;
    }

    if (ws_utt.message_type == WS_FT_EAPOL) {
        tr_extra_debug("calling ws_llc_eapol_indication_cb");
...
static void ws_llc_lowpan_mpx_data_request(llc_data_base_t *base, mpx_user_t *user_cb, const struct mcps_data_req_s *data, mac_data_priority_t priority)
{
...
#else
    uint16_t over_head_size = 5; //MPX FuLL frame 3 bytes + IE header 2 byte
#endif
    ...
    if (!message) {
        ...
        tr_extra_debug("ws_llc_lowpan_mpx_data_request - Transaction Overflow");
        user_cb->data_confirm(&base->mpx_data_base.mpx_api, &data_conf);
        return;
    }
...
#else
    message->ie_vector_list[0].ieBase = ptr;
    message->ie_vector_list[0].iovLen = 0;
    message->ie_vector_list[1].ieBase = ptr;
    message->ie_vector_list[1].iovLen = 0;
    message->ie_vector_list[2].ieBase = data->msdu;
    message->ie_vector_list[2].iovLen =  data->msduLength;
    ...
    tr_extra_debug("call mcps_data_req_ext");
    uint8_t status;

    status = base->interface_ptr->mac_api->mcps_data_req_ext(base->interface_ptr->mac_api, (void *)&data_req,
                                                    &message->ie_vector_list[1], &ie_header_mask, &nested_wp_id, NULL);
    if(status)
    {
...
#endif
{
    ...
    data_req.msduLength = message->ie_vector_list[2].iovLen;
    status = base->interface_ptr->mac_api->mcps_data_req_ext(base->interface_ptr->mac_api, (void *)&data_req,
                                                    &message->ie_vector_list[1], &message->ie_header_mask, &message->nested_wp_id, NULL);
    if(!status)
    {
        //Set active session to true only if packet is accepted by MAC
        base->temp_entries->active_eapol_session = true;
    }
...
static void ws_llc_mpx_eapol_request(llc_data_base_t *base, mpx_user_t *user_cb, const struct mcps_data_req_s *data, mac_data_priority_t priority)
{
    ...
    over_head_size += 5; //MPX FuLL frame 3 bytes + IE header 2 bytes
#else
    uint16_t over_head_size = 5; //MPX FuLL frame 3 bytes + IE header 2 bytes
#endif
...
static void ws_llc_mpx_data_request(const mpx_api_t *api, const struct mcps_data_req_s *data, uint16_t user_id, mac_data_priority_t priority)
{
    ...
    tr_extra_debug("ws_llc_mpx_data_request: enter");
...
#ifdef DBG_WISUN
        wisunDbg.req[WS_FT_EAPOL]++;
#endif
        tr_extra_debug("ws_llc_mpx_data_request: call ws_llc_mpx_eapol_request");
        ws_llc_mpx_eapol_request(base, user_cb, data, priority);

    } else if (user_id == MPX_LOWPAN_ENC_USER_ID) {
#ifdef DBG_WISUN
        wisunDbg.req[WS_FT_DATA]++;
#endif
        tr_extra_debug("ws_llc_mpx_data_request: call ws_llc_lowpan_mpx_data_request");
...
{
#ifdef DBG_WISUN
    wisunDbg.req[request->message_type]++;
#endif
    llc_data_base_t *base = ws_llc_discover_by_interface(interface);
    ...
    if (request->wp_requested_nested_ie_list.pan_version_ie) {
        //Update pan version
        timacStorePanVersionInformation(base->ie_params.pan_congiguration->pan_version);
    }
    /* TIMAC Limitation: No msdu handle support for async request.
     * Hence, limiting no of async request that can be sent a time to MAC to 1
     */
    if(last_sent_async_frame_type == 0xFF)
    {
        last_sent_async_frame_type = request->message_type;
        uint8_t status;
        status = base->interface_ptr->mac_api->mcps_data_req_ext(base->interface_ptr->mac_api, (void *)request, NULL, NULL, NULL, base->ie_params.gtkhash);
    ...
    {
        tr_error("\n Already one async at MAC: Dropping Async of type %d",request->message_type);
