...
int8_t ws_cfg_gen_set(protocol_interface_info_entry_t *cur)
{
    ws_cfg_t cfg = &ws_cfg;
    cur->ws_info->cfg->gen->network_size = NETWORK_SIZE_MEDIUM;
    strncpy(cur->ws_info->cfg->gen->network_name, new_cfg->network_name, 32);
    cur->ws_info->cfg->gen->network_pan_id = cfg->network_pan_id;
    cur->ws_info->cfg->gen->rpl_parent_candidate_max = cfg->rpl_parent_candidate_max;
    cur->ws_info->cfg->gen->rpl_selected_parent_max = cfg->rpl_selected_parent_max;
}

int8_t ws_cfg_timing_set(protocol_interface_info_entry_t *cur)
{
    ws_cfg_t cfg = &ws_cfg;

    cur->ws_info->trickle_params_pan_discovery.Imin = cfg->disc_trickle_imin * 10;
    cur->ws_info->trickle_params_pan_discovery.Imax = cfg->disc_trickle_imax * 10;
    cur->ws_info->trickle_params_pan_discovery.k = cfg->disc_trickle_k;
    cur->ws_info->trickle_params_pan_discovery.TimerExpirations = TRICKLE_EXPIRATIONS_INFINITE;
    return CFG_SETTINGS_OK;
}

int8_t ws_cfg_phy_set(protocol_interface_info_entry_t *cur)
{
    ws_cfg_t cfg = &ws_cfg;

    cur->ws_info->hopping_schdule.regulatory_domain = cfg->regulatory_domain;
    cur->ws_info->hopping_schdule.operating_mode = cfg->operating_mode;
    cur->ws_info->hopping_schdule.operating_class = cfg->operating_class;
...
int8_t ws_cfg_bbr_set(protocol_interface_info_entry_t *cur)
{
    ws_cfg_t cfg = &ws_cfg;
    ws_bbr_rpl_config(cur, cfg->dio_interval_min, cfg->dio_interval_doublings,
                      cfg->dio_redundancy_constant, cfg->dag_max_rank_increase,
                      cfg->min_hop_rank_increase);
    ws_bbr_dhcp_address_lifetime_set(cur, cfg->dhcp_address_lifetime);
    return CFG_SETTINGS_OK;
}

int8_t ws_cfg_mpl_set(protocol_interface_info_entry_t *cur)
{
    ws_cfg_t cfg = &ws_cfg;
    cur->mpl_data_trickle_params.Imin = MPL_MS_TO_TICKS(cfg->mpl_trickle_imin * 1000);
    cur->mpl_data_trickle_params.Imax = MPL_MS_TO_TICKS(cfg->mpl_trickle_imax * 1000);
    cur->mpl_data_trickle_params.k = cfg->mpl_trickle_k;
    cur->mpl_data_trickle_params.TimerExpirations = cfg->mpl_trickle_timer_exp;
    cur->mpl_seed_set_entry_lifetime = cfg->seed_set_entry_lifetime;
...
int8_t ws_cfg_sec_timer_set(protocol_interface_info_entry_t *cur)
{
    ws_cfg_t cfg = &ws_cfg;
    ws_pae_controller_configure(cur, cfg->sec_timer, NULL);
    return CFG_SETTINGS_OK;
}

int8_t ws_cfg_sec_prot_set(protocol_interface_info_entry_t *cur)
{
    ws_cfg_t cfg = &ws_cfg;
    ws_pae_controller_configure(cur, NULL, cfg->sec_prot);
    return CFG_SETTINGS_OK;
}
...
void ws_cfg_set_intferface_all(int8_t interface_id)
{
    cur = protocol_stack_interface_info_get_by_id(interface_id);
    ws_cfg_timing_set(cur);
    ws_cfg_phy_set(cur);
    ws_cfg_bbr_set(cur);
    ws_cfg_mpl_set(cur);
    ws_cfg_sec_timer_set(cur);
    ws_cfg_sec_prot_set(cur);
}
