        ...
        if (entry->timer <= ticks) {
            ...
            if (entry->wait_response) {
                ...
                if (entry->nud_process) {
                    ...
                    if (entry->retry_count < WS_NUD_RETRY_LIMIT) {
                        entry->timer = randLIB_get_random_in_range(1, 900);
                        entry->wait_response = false;
                    } else {
                ...
                if (!entry->wait_response) {
                    if (entry->nud_process && entry->retry_count < WS_NUD_RETRY_LIMIT) {
                        entry->timer = randLIB_get_random_in_range(1, 900);
                    } else {
                        //Clear entry from active list
                    ...
                    entry->retry_count++;
                    entry->timer = WS_NUD_RETRY_INTERVAL;
                }
            }
        } else {
            entry->timer -= ticks;
...
/* Recommend to double check the GTK Hash even for same PAN version (in case upstream node changed version just when it got the new keys) */
            return;
#endif
        } else  {
...
int ws_bootstrap_set_rf_config(protocol_interface_info_entry_t *cur, phy_rf_channel_configuration_s rf_configs)
{
    ...
    mac_helper_start_auto_cca_threshold(cur->id, cur->ws_info->hopping_schdule.number_of_channels, cfg_props.ccaDefaultdBm, CCA_HIGH_LIMIT, CCA_LOW_LIMIT);
    return 0;
}
...
static void ws_set_asynch_channel_list(protocol_interface_info_entry_t *cur, asynch_request_t *async_req)
{
    ...
    // Async channel list must be independent from unicast channel list because there is a use case
    // where someone may want their node to send unicast data on one channel while sending async frames to all channels.
    ...
    // TBD: put some trace line here
    uint8_t idx = 0, resIdx = 0, sizeOfChannelMask;
...
void ws_bootstrap_state_machine(protocol_interface_info_entry_t *cur)
{
    ...
    switch (cur->nwk_bootstrap_state) {
        ...
        case ER_PANA_AUTH:
            ...
            if ((new_default < cfg_props.ccaDefaultdBm) && (new_default >= CCA_LOW_LIMIT) && (new_default <= CCA_HIGH_LIMIT)) {
...
void ws_bootstrap_trickle_timer(protocol_interface_info_entry_t *cur, uint16_t ticks)
{
    ...
    if (cur->ws_info->trickle_pa_running || cur->ws_info->trickle_pc_running)
    {
        cur->ws_info->trickle_params_pan_discovery.Imax = 600;
