...
typedef struct {
    ...
    pae_supp_t* global_pae_supp = NULL;
#endif
...
#else
#define LIFETIME_MISMATCH_RETRY_COUNT      3   /* three retries */
#endif
...
static int8_t ws_pae_supp_initial_key_send(pae_supp_t *pae_supp)
{
    if (!pae_supp->auth_requested) {
        ...
        if (ws_pae_supp_parent_eui_64_get(pae_supp->interface_ptr, parent_eui_64) >= 0) {
            ...
            // No target, failure
            tr_extra_debug("No Parent or EAPOl-target found");
            return -1;
        }
...
static int8_t ws_pae_supp_parent_eui_64_get(protocol_interface_info_entry_t *interface_ptr, uint8_t *eui_64)
{
    ...
    if (!interface_ptr->rpl_domain) {
        tr_extra_debug("\n ws_pae_supp_parent_eui_64_get: no rpl domain");
        return -1;
    }
    ...
    if (instance && rpl_control_read_dodag_info(instance, &dodag_info)) {
        ...
        if (parent_ll_addr) {
            tr_extra_debug("\n parent_ll_address %s", trace_array(parent_ll_addr,8));
        ...
        {
            tr_extra_debug("\n No local pref parent address");
        }
    }
    else
    {
        tr_extra_debug("\n RPL Instance is null. No valid parent");
    }
