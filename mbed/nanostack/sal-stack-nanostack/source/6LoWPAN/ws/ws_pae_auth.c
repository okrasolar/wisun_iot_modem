...
#if !defined(GTK_CERT_TEST) && !defined(GTK_CERT_TEST_FULL_CYCLE) && !defined(GTK_CERT_BR_REVOKE_TEST)
        //Insert second key to add the 2-way GTK Cycle
        ws_pae_auth_gtk_key_insert(pae_auth);
#endif
#endif
    } else {
...
int8_t ws_pae_auth_node_access_revoke_start(protocol_interface_info_entry_t *interface_ptr)
{
    ...
    if (active_index >= 0) {
        ...
        tr_info("Active Lifetime: %i"PRIu32", revocation_lifetime: %"PRIu32"", active_lifetime, revocation_lifetime);
...
static void ws_pae_auth_gtk_key_insert(pae_auth_t *pae_auth)
{
    ...
    if (next_gtk_index >= 0) {
        ...
        if(!gtk)
        {
            tr_info("ws_pae_auth_gtk_key_insert: Null gtk pointer");
            return;
        }
