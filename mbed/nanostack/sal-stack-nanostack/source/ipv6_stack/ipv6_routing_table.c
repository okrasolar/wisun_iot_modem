...
extern void nanostack_process_routing_table_update_from_stack(uint8_t changed_info, uint8_t* prefix, uint8_t len_prefix, uint8_t* addr_nexthop, uint32_t lifetime);
...
{
    uint_fast16_t gc_count = 0;
    ipv6_neighbour_cache_t * neighbour_cache = NULL;
        ...
        if (entry->pmtu_lifetime) {
            if (entry->pmtu_lifetime <= DCACHE_GC_PERIOD) {
                ...
                if (entry->interface_id >= 0) {
                    neighbour_cache = ipv6_neighbour_cache_by_interface_id(entry->interface_id);
                    if(!neighbour_cache)
                    {
                        return;
                    }
                    entry->pmtu = neighbour_cache->link_mtu;
                } else {
                    entry->pmtu = 0xffff;
                }
...
static void ipv6_route_print(const ipv6_route_t *route, route_print_fn_t *print_fn)
{
    ...
    if(route && print_fn && route->info.source < ROUTE_MAX)
    {
...
ipv6_route_t *ipv6_route_add_metric(const uint8_t *prefix, uint8_t prefix_len, int8_t interface_id, const uint8_t *next_hop, ipv6_route_src_t source, void *info, uint8_t source_id, uint32_t lifetime, uint8_t metric)
{
    ...
    ipv6_route_update_t changed_info = UNCHANGED;
