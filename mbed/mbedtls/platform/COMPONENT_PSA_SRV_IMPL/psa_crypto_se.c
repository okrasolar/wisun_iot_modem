    ...
    psa_key_slot_number_t* slot_number)
{
    ...
    if (psa_get_key_slot_number(attributes, slot_number) == PSA_SUCCESS) {
        ...
        psa_drv_se_validate_slot_number_t p_validate_slot_number = driver->methods->key_management->p_validate_slot_number;
    ...
    psa_key_slot_number_t slot_number)
{
    ...
    if (driver->methods->key_management == NULL || driver->methods->key_management->p_destroy == NULL)
    ...
    const psa_drv_se_t* methods)
{
    ...
    driver_table[i].internal.persistent_data_size = methods->persistent_data_size;

    if (methods->persistent_data_size != 0) {
        driver_table[i].internal.persistent_data = mbedtls_calloc(1, methods->persistent_data_size);
