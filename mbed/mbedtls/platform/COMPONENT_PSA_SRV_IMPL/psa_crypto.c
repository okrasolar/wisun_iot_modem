    ...
    psa_algorithm_t alg2)
{
    ...
    if (PSA_ALG_IS_HASH_AND_SIGN(alg1) && PSA_ALG_IS_HASH_AND_SIGN(alg2) && (alg1 & ~PSA_ALG_HASH_MASK) == (alg2 & ~PSA_ALG_HASH_MASK)) {
...
psa_status_t psa_destroy_key(psa_key_handle_t handle)
{
    ...
    if (slot->attr.lifetime != PSA_KEY_LIFETIME_VOLATILE) {
        ...
        /* Other slots may have a copy of the same key. We should
         * invalidate them.
    ...
    psa_key_attributes_t* attributes)
{
    ...
    attributes->core.flags &= (MBEDTLS_PSA_KA_MASK_EXTERNAL_ONLY | MBEDTLS_PSA_KA_MASK_DUAL_USE);
...
#if defined(MBEDTLS_PSA_CRYPTO_SE_C)
            /* Reporting the public exponent for opaque keys
    ...
    *data_length = 0;
    ...
    if (psa_get_se_driver(slot->attr.lifetime, &drv, &drv_context)) {
        ...
        if (drv->key_management == NULL)
            return (PSA_ERROR_NOT_SUPPORTED);
        method = (export_public_key ? drv->key_management->p_export_public : drv->key_management->p_export);
        if (method == NULL)
            return (PSA_ERROR_NOT_SUPPORTED);
    ...
    if (PSA_KEY_TYPE_IS_ECC_KEY_PAIR(slot->attr.type) && !export_public_key) {
        ...
        *data_length = bytes;
        ...
        if (PSA_KEY_TYPE_IS_RSA(slot->attr.type) || PSA_KEY_TYPE_IS_ECC(slot->attr.type)) {
            mbedtls_pk_context pk;
            int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
            if (PSA_KEY_TYPE_IS_RSA(slot->attr.type)) {
#if defined(MBEDTLS_RSA_C)
...
{
    if ((policy->usage & ~(PSA_KEY_USAGE_EXPORT | PSA_KEY_USAGE_COPY | PSA_KEY_USAGE_ENCRYPT | PSA_KEY_USAGE_DECRYPT | PSA_KEY_USAGE_SIGN_HASH | PSA_KEY_USAGE_VERIFY_HASH | PSA_KEY_USAGE_DERIVE)) != 0)
        return (PSA_ERROR_INVALID_ARGUMENT);
    ...
    psa_se_drv_table_entry_t** p_drv)
{
    ...
    if (attributes->core.flags & ~(MBEDTLS_PSA_KA_MASK_EXTERNAL_ONLY | MBEDTLS_PSA_KA_MASK_DUAL_USE))
        return (PSA_ERROR_INVALID_ARGUMENT);
    ...
    psa_se_drv_table_entry_t* driver)
{
...
#if defined(static_assert)
            static_assert(sizeof(slot->data.se.slot_number) == sizeof(data.slot_number),
...
#endif /* MBEDTLS_PSA_CRYPTO_SE_C */
        {
            size_t buffer_size = PSA_KEY_EXPORT_MAX_SIZE(slot->attr.type,
                slot->attr.bits);
    ...
    if (driver != NULL && psa_crypto_transaction.unknown.type == PSA_CRYPTO_TRANSACTION_CREATE_KEY) {
        status = psa_save_se_persistent_data(driver);
        if (status != PSA_SUCCESS) {
...
#if defined(MBEDTLS_PSA_CRYPTO_SE_C)
    /* If the key has already been created in the secure
    ...
    int is_sign)
{
    ...
    psa_key_slot_t* slot;
    size_t key_bits;
    psa_key_usage_t usage = is_sign ? PSA_KEY_USAGE_SIGN_HASH : PSA_KEY_USAGE_VERIFY_HASH;
    ...
    if (full_length_alg == PSA_ALG_CMAC) {
        const mbedtls_cipher_info_t* cipher_info = mbedtls_cipher_info_from_psa(full_length_alg,
        ...
        if (operation->mac_size == 0 || operation->mac_size > sizeof(operation->ctx.hmac.opad)) {
            status = PSA_ERROR_NOT_SUPPORTED;
            goto exit;
        }
    ...
    mbedtls_md_type_t* md_alg)
{
    ...
    *md_alg = mbedtls_md_get_type(md_info);
     ...
     * must be correct. */
    if (PSA_ALG_IS_RSA_PKCS1V15_SIGN(alg) && alg != PSA_ALG_RSA_PKCS1V15_SIGN_RAW) {
        if (md_info == NULL)
            return (PSA_ERROR_NOT_SUPPORTED);
    ...
    size_t* output_length)
{
    ...
    *output_length = 0;
    ...
    if (status != PSA_SUCCESS)
        return (status);
    if (!(PSA_KEY_TYPE_IS_PUBLIC_KEY(slot->attr.type) || PSA_KEY_TYPE_IS_KEY_PAIR(slot->attr.type)))
        return (PSA_ERROR_INVALID_ARGUMENT);
    ...
    mbedtls_operation_t cipher_operation)
{
    ...
    psa_key_usage_t usage = (cipher_operation == MBEDTLS_ENCRYPT ? PSA_KEY_USAGE_ENCRYPT : PSA_KEY_USAGE_DECRYPT);
    ...
    operation->key_set = 1;
    operation->block_size = (PSA_ALG_IS_STREAM_CIPHER(alg) ? 1 : PSA_BLOCK_CIPHER_BLOCK_SIZE(slot->attr.type));
    ...
    size_t* output_length)
{
    ...
    if (operation->ctx.cipher.operation == MBEDTLS_ENCRYPT && operation->alg == PSA_ALG_CBC_NO_PADDING && operation->ctx.cipher.unprocessed_len != 0) {
        status = PSA_ERROR_INVALID_ARGUMENT;
        goto error;
    }
    ...
    psa_algorithm_t alg)
{
    ...
    operation->cipher_info = mbedtls_cipher_info_from_psa(alg, operation->slot->attr.type, key_bits,
        &cipher_id);
    ...
    size_t output_length)
{
    ...
    } else if (PSA_ALG_IS_TLS12_PRF(kdf_alg) || PSA_ALG_IS_TLS12_PSK_TO_MS(kdf_alg)) {
...
#if defined(MBEDTLS_MD_C)
    if (PSA_ALG_IS_HKDF(kdf_alg) || PSA_ALG_IS_TLS12_PRF(kdf_alg) || PSA_ALG_IS_TLS12_PSK_TO_MS(kdf_alg)) {
        psa_algorithm_t hash_alg = PSA_ALG_HKDF_GET_HASH(kdf_alg);
        size_t hash_size = PSA_HASH_SIZE(hash_alg);
        if (hash_size == 0)
            return (PSA_ERROR_NOT_SUPPORTED);
        if ((PSA_ALG_IS_TLS12_PRF(kdf_alg) || PSA_ALG_IS_TLS12_PSK_TO_MS(kdf_alg)) && !(hash_alg == PSA_ALG_SHA_256 || hash_alg == PSA_ALG_SHA_384)) {
            return (PSA_ERROR_NOT_SUPPORTED);
        }
    ...
    const uint8_t* domain_parameters, size_t domain_parameters_size)
{
    ...
    if (key_type_is_raw_bytes(type)) {
        ...
        mbedtls_ecp_group_id grp_id = mbedtls_ecc_group_of_psa(curve, PSA_BITS_TO_BYTES(bits));
    ...
    const psa_crypto_transaction_t* transaction)
{
    switch (transaction->unknown.type) {
        ...
        case PSA_CRYPTO_TRANSACTION_DESTROY_KEY:
            /* Fall through to the failure case until this
             * is implemented.
