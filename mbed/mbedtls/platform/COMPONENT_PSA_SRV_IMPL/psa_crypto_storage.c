...
{
#if defined(MBEDTLS_PSA_CRYPTO_KEY_FILE_ID_ENCODES_OWNER) && defined(PSA_CRYPTO_SECURE)
...
{
    psa_persistent_key_storage_format* storage_format = (psa_persistent_key_storage_format*)storage_data;
...
{
    psa_status_t status;
    const psa_persistent_key_storage_format* storage_format = (const psa_persistent_key_storage_format*)storage_data;
    uint32_t version;
    uint32_t type;
    ...
    if (*key_data_length > (storage_data_length - sizeof(*storage_format)) || *key_data_length > PSA_CRYPTO_MAX_STORAGE_SIZE)
        return (PSA_ERROR_STORAGE_FAILURE);
