...
#if defined(MBEDTLS_SSL_SOME_MODES_USE_MAC) && (defined(MBEDTLS_SSL_PROTO_TLS1) || defined(MBEDTLS_SSL_PROTO_TLS1_1) || defined(MBEDTLS_SSL_PROTO_TLS1_2))
    ...
    uint8_t rec_type)
{
    ...
    size_t pad = (MBEDTLS_SSL_CID_PADDING_GRANULARITY - (len + 1) % MBEDTLS_SSL_CID_PADDING_GRANULARITY) % MBEDTLS_SSL_CID_PADDING_GRANULARITY;
    ...
    void* p_rng)
{
...
#if !(defined(MBEDTLS_CIPHER_MODE_CBC) && (defined(MBEDTLS_AES_C) || defined(MBEDTLS_ARIA_C) || defined(MBEDTLS_CAMELLIA_C)) && (defined(MBEDTLS_SSL_PROTO_TLS1_1) || defined(MBEDTLS_SSL_PROTO_TLS1_2)))
    ((void)f_rng);
    ((void)p_rng);
#endif
    ...
    if (rec->data_len > MBEDTLS_SSL_OUT_CONTENT_LEN) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("Record content %u too large, maximum %d", (unsigned)rec->data_len, MBEDTLS_SSL_OUT_CONTENT_LEN));
    ...
    if (mode == MBEDTLS_MODE_STREAM) {
        ...
        if (post_avail < transform->taglen || rec->data_offset < explicit_iv_len) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("Buffer provided for encrypted record not large enough"));
            return (MBEDTLS_ERR_SSL_BUFFER_TOO_SMALL);
        }
...
#if defined(MBEDTLS_CIPHER_MODE_CBC) && (defined(MBEDTLS_AES_C) || defined(MBEDTLS_CAMELLIA_C) || defined(MBEDTLS_ARIA_C))
    ...
    mbedtls_record* rec)
{
    ...
    if (mode == MBEDTLS_MODE_STREAM) {
        ...
        if (rec->data_len % transform->ivlen != 0) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("msglen (%d) %% ivlen (%d) != 0", rec->data_len, transform->ivlen));
            return (MBEDTLS_ERR_SSL_INVALID_MAC);
        }
        ...
        if (auth_done == 1) {
            ...
            if (rec->data_len < transform->maclen + padlen + 1) {
                MBEDTLS_SSL_DEBUG_MSG(1, ("msglen (%d) < maclen (%d) + padlen (%d)", rec->data_len, transform->maclen, padlen + 1));
            }
#endif
...
#if defined(MBEDTLS_MD5_C) || defined(MBEDTLS_SHA1_C) || defined(MBEDTLS_SHA256_C)
                case MBEDTLS_MD_MD5:
                case MBEDTLS_MD_SHA1:
                case MBEDTLS_MD_SHA256:
                    /* 8 bytes of message size, 64-byte compression blocks */
                    extra_run = (add_data_len + rec->data_len + padlen + 8) / 64 - (add_data_len + rec->data_len + 8) / 64;
                    break;
#endif
#if defined(MBEDTLS_SHA512_C)
                case MBEDTLS_MD_SHA384:
                    /* 16 bytes of message size, 128-byte compression blocks */
                    extra_run = (add_data_len + rec->data_len + padlen + 16) / 128 - (add_data_len + rec->data_len + 16) / 128;
                    break;
#endif
                default:
                    MBEDTLS_SSL_DEBUG_MSG(1, ("should never happen"));
...
static int ssl_compress_buf(mbedtls_ssl_context* ssl)
{
    ...
    ssl->out_msglen = out_buf_len - ssl->transform_out->ctx_deflate.avail_out - bytes_written;
...
static int ssl_decompress_buf(mbedtls_ssl_context* ssl)
{
    ...
    ssl->in_msglen = in_buf_len - ssl->transform_in->ctx_inflate.avail_out - header_bytes;
...
int mbedtls_ssl_flush_output(mbedtls_ssl_context* ssl)
{
    ...
    while (ssl->out_left > 0) {
        MBEDTLS_SSL_DEBUG_MSG(2, ("message length: %d, out_left: %d", mbedtls_ssl_out_hdr_len(ssl) + ssl->out_msglen, ssl->out_left));
...
int mbedtls_ssl_write_handshake_msg(mbedtls_ssl_context* ssl)
{
    ...
    if (ssl->out_msgtype != MBEDTLS_SSL_MSG_HANDSHAKE && ssl->out_msgtype != MBEDTLS_SSL_MSG_CHANGE_CIPHER_SPEC) {
        ...
        if (!(ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_0 && ssl->out_msgtype == MBEDTLS_SSL_MSG_ALERT && ssl->conf->endpoint == MBEDTLS_SSL_IS_CLIENT))
...
#if defined(MBEDTLS_SSL_PROTO_DTLS)
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM && !(ssl->out_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE && hs_type == MBEDTLS_SSL_HS_HELLO_REQUEST)) {
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        ...
        if (ssl->handshake != NULL && ((ssl->state != MBEDTLS_SSL_HANDSHAKE_OVER && recv_msg_seq != ssl->handshake->in_msg_seq) || (ssl->state == MBEDTLS_SSL_HANDSHAKE_OVER && ssl->in_msg[0] != MBEDTLS_SSL_HS_CLIENT_HELLO))) {
            if (recv_msg_seq > ssl->handshake->in_msg_seq) {
                MBEDTLS_SSL_DEBUG_MSG(2, ("received future handshake message of sequence number %u (next %u)", recv_msg_seq, ssl->handshake->in_msg_seq));
    ...
    unsigned char* obuf, size_t buf_len, size_t* olen)
{
    ...
    if (f_cookie_write(p_cookie,
            &p, obuf + buf_len, cli_id, cli_id_len)
        != 0) {
        return (MBEDTLS_ERR_SSL_INTERNAL_ERROR);
    }
    ...
    mbedtls_record* rec)
{
    ...
    if (len < rec_hdr_len_offset + rec_hdr_len_len) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("datagram of length %u too small to hold DTLS record header of length %u", (unsigned)len, (unsigned)(rec_hdr_len_len + rec_hdr_len_len)));
        return (MBEDTLS_ERR_SSL_INVALID_RECORD);
    }
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM && ssl->conf->cid_len != 0 && rec->type == MBEDTLS_SSL_MSG_CID) {
        ...
        if (len < rec_hdr_len_offset + rec_hdr_len_len) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("datagram of length %u too small to hold DTLS record header including CID, length %u", (unsigned)len, (unsigned)(rec_hdr_len_offset + rec_hdr_len_len)));
            return (MBEDTLS_ERR_SSL_INVALID_RECORD);
        }
    ...
    rec->data_len = ((size_t)buf[rec_hdr_len_offset + 0] << 8) | ((size_t)buf[rec_hdr_len_offset + 1] << 0);
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        ...
        if (len < rec->data_offset + rec->data_len) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("Datagram of length %u too small to contain record of advertised length %u.", (unsigned)len, (unsigned)(rec->data_offset + rec->data_len)));
            return (MBEDTLS_ERR_SSL_INVALID_RECORD);
        }
    ...
    mbedtls_record* rec)
{
    ...
    if (!done && ssl->transform_in != NULL) {
        ...
        if (old_msg_type != rec->type) {
            MBEDTLS_SSL_DEBUG_MSG(4, ("record type after decrypt (before %d): %d", old_msg_type, rec->type));
        }
    ...
    /* Debug only */
    {
        ...
        for (offset = 1; offset < MBEDTLS_SSL_MAX_BUFFERED_HS; offset++) {
            ...
            if (hs_buf->is_valid == 1) {
                MBEDTLS_SSL_DEBUG_MSG(2, ("Future message with sequence number %u %s buffered.", hs->in_msg_seq + offset, hs_buf->is_complete ? "fully" : "partially"));
            }
        }
    }
