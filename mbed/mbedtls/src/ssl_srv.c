    ...
    const mbedtls_ssl_ciphersuite_t** ciphersuite_info)
{
    ...
    if (ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3) {
        ...
        if (sig_type != MBEDTLS_PK_NONE && mbedtls_ssl_sig_hash_set_find(&ssl->handshake->hash_algs, sig_type) == MBEDTLS_MD_NONE) {
...
read_record_header:
...
#endif
    {
        ...
        if ((ret = mbedtls_ssl_fetch_input(ssl,
                 mbedtls_ssl_in_hdr_len(ssl) + msg_len))
            != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_ssl_fetch_input", ret);
            return (ret);
        }
    ...
    size_t* olen)
{
    ...
    if (ssl->session_negotiate->encrypt_then_mac == MBEDTLS_SSL_ETM_DISABLED || ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_0) {
        *olen = 0;
        return;
    }
...
static int ssl_write_certificate_request(mbedtls_ssl_context* ssl)
{
    ...
    if (!mbedtls_ssl_ciphersuite_cert_req_allowed(ciphersuite_info) || authmode == MBEDTLS_SSL_VERIFY_NONE) {
        MBEDTLS_SSL_DEBUG_MSG(2, ("<= skip write certificate request"));
        return (0);
    }
...
#if defined(MBEDTLS_KEY_EXCHANGE_WITH_SERVER_SIGNATURE_ENABLED) && defined(MBEDTLS_SSL_ASYNC_PRIVATE)
    ...
    size_t* signature_len)
{
    ...
    if (mbedtls_ssl_ciphersuite_uses_server_signature(ciphersuite_info)) {
        ...
        if (ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3) {
            ...
            if (sig_alg == MBEDTLS_PK_NONE || (md_alg = mbedtls_ssl_sig_hash_set_find(&ssl->handshake->hash_algs, sig_alg)) == MBEDTLS_MD_NONE) {
                MBEDTLS_SSL_DEBUG_MSG(1, ("should never happen"));
...
#if defined(MBEDTLS_SSL_ASYNC_PRIVATE) && (defined(MBEDTLS_KEY_EXCHANGE_RSA_ENABLED) || defined(MBEDTLS_KEY_EXCHANGE_RSA_PSK_ENABLED))
    if ((ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_RSA_PSK || ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_RSA) && (ssl->handshake->async_in_progress != 0)) {
